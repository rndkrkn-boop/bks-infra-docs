...
2. Полнота не отслеживалась. `SKIP: board not yet created` не инкрементировал
   ...
   `complete` выставляется только при полном совпадении с политикой
   (`backup/retention.toml`, N/N обязательных артефактов — число сознательно
   не захардкожено здесь, чтобы этот комментарий не расходился с политикой
   при следующем изменении состава).
