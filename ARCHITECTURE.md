...
## 2.5 K3s: деплой сервисов (✅ работает с 2026-07-01)
...
│  │    env: SECURITY_MODEL=security-lora-v1                                  │  │
...
**K3s задеплоен (2026-07-01):**
- агенты переключаются на `BKS_ROUTER_URL=http://192.168.2.180:30400`
- docker-compose роутер (`router-proxy`, `router-vllm-classifier`) работает параллельно до переключения
