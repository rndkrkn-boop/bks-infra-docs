...
> **Статус 2026-07-02:** K8s router удалён. Роутер работает через docker-compose (port 4000).

> **Статус 2026-07-03:** vllm-classifier (security/PII LoRA) полностью выведен
> из K3s. Аудит показал, что после удаления K8s router 2026-07-02 у пода
> vllm-classifier в кластере не осталось `Service` (манифест его описывал,
> но в кластере объекта не было — `vllm-classifier.bks-router.svc` резолвился
> в NXDOMAIN), так что ни один прод-запрос через него не проходил: docker-compose
> роутер обращался к своему собственному локальному `vllm-classifier` без LoRA,
> и `SECURITY_MODEL` был пуст → `security_check()` в classifier.py молча
> пропускался на каждом запросе. Т.к. это единственный экземпляр на этом
> железе (GB10, один GPU) — K8s-репликация ему ничего не даёт — сервис
> перенесён целиком в docker-compose на хосте (см. §2 выше:
> `router/docker-compose.yml`, сервис `vllm-classifier`, volume
> `/home/admin/models/adapters:/adapters:ro`, флаги `--enable-lora
> --lora-modules security-lora-v1=... pii-cleaner-lora-v1=...`).
> `SECURITY_MODEL=security-lora-v1` теперь задан в `router/.env`.
> Namespace `bks-router` в K3s больше не содержит рабочих нагрузок (Deployment
> и все ReplicaSet удалены); манифест `deploy/01-vllm-classifier.yaml`
> сохранён в репозитории как документация GB10-обхода, помечен deprecated.

```
┌──────────────────────────────────────────────────────────────────────────────┐
│  К3s  (однонодовый, на хосте 192.168.2.180)                                 │
│                                                                               │
│  namespace: bks-router — ПУСТ (2026-07-03). vllm-classifier перенесён       │
│  на хост, в docker-compose (см. §2). Secret router-apikeys — не используется.│
│                                                                               │
...
│    → CrashLoopBackOff; ничего в K3s больше не запрашивает GPU (vllm-classifier│
│      перенесён на хост) — daemonset можно удалить, а не чинить              │
│                                                                               │
