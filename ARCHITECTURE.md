...
| `matrix/` | Self-hosted Matrix Synapse + PostgreSQL — задел под внутренний LAN-чат/ops-алерты без зависимости от `api.telegram.org`, federation выключена. С 2026-08-07 доведён до паттерна соседей: свой проект `bks/matrix` в GitLab (ветка `main`), минимальный CI (`.gitlab-ci.yml`: shellcheck + compose-lint, без deploy-стадии), в daily-backup контракте host-infra (`matrix-synapse.tar.gz` + `matrix-postgres.sql.gz` через `pg_dump`, 10/10 обязательных артефактов). Уже работает на хосте (Synapse+PG живые, боты `director-bot`/`mkt-bot` зарегистрированы в Synapse, network policy `host.openshell.internal:8008` на sandbox уже применена), но `platforms.matrix` ни в один профильный `config.yaml` всё ещё не подключён — единственный оставшийся пункт R13 в [аудите 2026-07-25](./docs/audit/full-project-audit-2026-07-25.md). |
