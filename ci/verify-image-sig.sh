...
#
# Статус в этом репозитории: не вызывается ни .gitlab-ci.yml, ни
# автономным daily-циклом — только scripts/cosign-selftest.sh проверяет,
# что гейт действительно fail-closed. Почему и кто должен подключать:
# docs/ci-pipeline-notes.md.
#
