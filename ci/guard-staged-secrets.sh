...
# Носители секретов по определению: .env в любом каталоге (и как есть, и как
# суффикс/расширение — `.env`, `.env.local`, но и `database.env`, `myapp.env`,
# которые старый паттерн `(^|/)\.env` пропускал: он требовал точку сразу после
# `/` или начала строки, а перед `.env` в "database.env" стоит буква, не `/`),
# приватные ключи и сертификаты, ssh-ключи. Совпадение по ИМЕНИ, а не по
# содержимому: содержимое проверяет secrets-scan (gitleaks) в .gitlab-ci.yml,
# здесь дешёвый и детерминированный барьер, который нельзя обойти
# изобретательным форматом.
FORBIDDEN='(^|/)\.env(\.[^/]*)?$|(^|/)[^/]+\.env$|\.pem$|\.key$|id_rsa'
...
if [ ${#offenders[@]} -gt 0 ]; then
    ...
    echo "   проверить .gitignore и compliance/rules.toml (правило SEC-003)" >&2
