...
- Не пишем свой memory-провайдер Hermes для MemGraphRAG (интеграция через
  MCP-сервер — `bksamotsvety/mcp/memgraphrag_mcp.py`, стандартный клиент
  `hermes mcp` — проще, чем провайдер, и уже реализована; official-провайдеры
  Hermes — honcho/mem0/… — не заменяют доменный граф).
